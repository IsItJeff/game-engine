...
// Create a MIRE — a boggy patch that DRAGS on movement (integrate_motion scales the crossing
// mover's position delta by slow_factor). Neutral terrain, not a hazard: it doesn't hurt, it just
// slows — so it reshapes a chase or a flight rather than threatening. A murky brown disc drawn at
// its reach. Scenery — no Stats/Velocity.
...
entt::entity build_scene(entt::registry& reg, std::mt19937& rng) {
  ...
  // A MIRE to the WEST — a murky bog that DRAGS on anyone crossing it (integrate_motion), the
  // movement twin of the cold zone's Need-drain. Set well off the player's centre spawn (its near
  // edge clears it), so you start on firm ground and choose when to slog through — and so a
...
void World::step() {
  ...
  integrate_motion(registry_,
                   dt);           // apply velocity -> position (a MireZone drags the mover it's in)
  ...
  npc_haste(registry_);          // ...or QUICKENS itself to flee/reposition (the fifth verb)...
