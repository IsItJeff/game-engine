...
×1, so it too is bit-identical until foraging trains it. That *same* `awareness` factor also widens
the **forage reach** (`kForageRadius × awareness`), the opportunity twin of the danger sense: a
wise forager spots a meal from farther — and since foraging is what *trains* WIS, the land-wise
colonist finds food a novice would walk past, compounding. (WIS 1 → ×1 → the base reach → every
forage test bit-identical.) A **third** factor is social — **courage in
...
which dream it carries — **four kinds** are wired, and `-Wswitch` guards against a future one forgetting
its rung.
...
steers toward the nearest **ripe** food plot within `kTendRange` — one grown to at least `kHarvestCost`,
the *same* ripeness bar its harvest uses, so it never walks to a barely-grown patch it couldn't
actually reap and leaves a ripe one it could (the seek and the reap read one shared threshold). Once it
arrives **`npc_harvest`** reaps that ripe plot into a meal — the same actor-agnostic
[`harvest_nearest_crop`](stats-system.md) the player's **G** command calls, so a colonist farms exactly
as you do (the design's promised *NPC farm behaviour*). It's **self-throttled**: harvesting spends the
plot's stock below the ripeness bar, so a provider reaps a patch then must wait for it to regrow —
meals trickle out to feed the colony rather than a firehose. And a provider doesn't only *reap*:
standing where it has **no plot to tend** (none within a sow-clear range), it **sows** one with the
same [`plant_crop`](stats-system.md) your **T** command uses — so a barren stretch gets seeded, grows,
and is later reaped. One spacing guard does double duty: a provider heading to a distant ripe plot is
already within range of it (so it won't sow a redundant one), and one beside a growing or just-picked
patch tends *that* rather than carpeting the field; plots settle roughly a range apart. So the food
economy grows a *producer* who **plants as well as harvests** — the front of the **plant → grow →
harvest → meal** chain, now NPC-driven — not just consumers.
...
The **`Healer`** dreams of **mercy** — the support twin. An idle **compassionate** colonist rushes to the
nearest **hurt** ally (one that's *wounded*, health below its max, **or** *poisoned*) within `kTendRange`,
so [`npc_heal` / `npc_cleanse`](magic.md) can reach it: a compassionate **caster** becomes a proactive
**field medic**, *seeking* the hurt rather than only tending whoever wanders into range. It supplies the
**intent** (bring the hurt ally into reach), not the mend itself — so a non-caster's rush carries no
mend *yet*: it's the compassionate instinct alone (be near the hurt), the **one** aspiration whose
*healing* payoff awaits a prerequisite (the magic — which a Healer, *unlike* the Scholar, doesn't itself
go and learn, so a non-caster Healer only becomes a medic by chance-wandering onto a tome), rather than resolving on arrival like the hunt lands
blows, the harvest reaps, and the study learns. It tends the **player** too. Mostly self-limiting: a
badly-hurt healer retreats to a hearth first if one is in reach (the rung above) — but with no fire
nearby it may still rush to the hurt, a selfless streak.

All four dreams sit **above** the idle rally/bond/gather rungs (a colonist chases its dream rather than
...
(but not brave) one the `Provider`, a **greedy** (but neither) one the `Scholar`, and a **compassionate**
(but none of those) one the `Healer` — keyed on the
...
boggy patch of mud — is the first piece of terrain that touches movement: `integrate_motion` scales the
...
creature, and ambient mote alike** — and even a **power-swing knockback** shoving a foe standing in it
(see [combat](combat.md), the raw factor since a passive shove doesn't *wade*) — no one is *immune* —
which makes it *tactical* from both
...
way (to within float rounding), and every velocity-reading system downstream (`update_stamina`, `drain_hunger` — both plain
*moving-vs-still*) still sees it as **moving**, so a mired crawler pays full exertion for little ground.
Overlapping mires don't stack — the **stickiest** (smallest `slow_factor`) wins, applied once, so it
stays deterministic. With **no `MireZone`** placed the factor is `1.0` and `position += velocity · dt`
exactly — a world without mud moves as before, bit-identical.
...
### Terrain that bites: a hazard field chips health

The mire *slows*; a **`HazardField`** *hurts*. It's the direct-HP twin of the [cold zone](stats-system.md)'s
warmth drain — a bramble bed or scald patch: a disc that **chips `damage_per_second` HEALTH** from
everyone standing inside it each tick (`tick_hazard_fields`, run in `step()` right after the need
drains). Where the mire's drag is neutral and the cold zone's chill routes through a Need, this bites
HP **outright**, and it bites **friend and foe alike** — the victim view excludes only the **Downed**
(inert, like every need tick), *not* creatures. So a hazard field is terrain you can turn into a
**weapon**: lead a charging brute across the thorns and they wear themselves down, the biting cousin of
kiting one through the mud. A victim dropped to 0 routes through the **normal death path**
(`handle_deaths`) — a player Downs, an NPC or creature permadies — exactly as starving does. Overlapping
fields don't stack — the **worst** (deadliest `damage_per_second`) bites, applied once, exactly as the
mire's **stickiest** drag wins. With **no `HazardField`** placed the field view is
empty and the early-out returns before touching anyone, so a world without brambles is **bit-identical**
and it draws no RNG. `build_scene` seeds one north of centre, off the player's spawn.
...
- `tests/sim/test_simulation.cpp` — flee / forage / rescue / revive-in-place, steer-to-weapon / NPC-arms-itself / armed-NPC-flees-slower (the equip bane parity), the villain-fear reader (a colonist flees a Suspect+ player; a downed villain is neither feared nor rescued — the villain-veto, in both steer and `handle_deaths`), and its rally twin (an idle colonist gathers to a Known+ hero, a real need overrides it, and below the line nobody is pulled), and `sociability` scaling how far an idle colonist travels to rally, the **warrior-hunt** (an idle `Warrior` charges the nearest creature; without the aspiration it stays put; and fear outranks the hunt), the **provider** (an idle `Provider` walks to the nearest RIPE plot, not a nearer unripe one it couldn't reap; `npc_harvest` reaps a ripe plot into a meal for a Provider but not a plain colonist; and a Provider on barren ground SOWS a seedling while one beside a plot sows none — the spacing guard), the **scholar** (an idle `Scholar` seeks a `Spellbook` to learn Spellcasting; one that already carries it stops seeking), the **healer-mercy** (an idle `Healer` rushes to a hurt ally — wounded or poisoned — while without the aspiration it stays put), and the **mire** (a mover crawls a fraction of the distance inside a bog but keeps its velocity; overlapping mires apply the stickiest factor once; and an un-re-driven mover crawls through and exits rather than freezing), and the **hazard field** (a Stats-bearer inside a bramble patch loses health each tick, a creature included — the kite value; one outside is untouched, the already-Downed are spared, and overlapping fields don't stack — the deadliest bites).
